<template>
  <div class="flex-1 p-8">
    <div
      class="bg-white rounded-lg shadow-lg overflow-hidden max-w-7xl mx-auto"
    >
      <table class="w-full">
              ...
              <div class="w-full">
                ...
                <div v-else>
                  <input
                    ...
                    @click.stop
                    class="mt-2 text-black px-2 py-1 border rounded"
                    placeholder="Search Accounts"
                  />
                  <button class="text-lg" @click="toggleSearch('name')">
                    &#x24E7;
                  </button>
                </div>
              </div>
              ...
              <div class="w-full">
                ...
                <div v-else>
                  <input
                    ...
                    placeholder="Search Accounts"
                  />
                  <button class="text-lg" @click="toggleSearch('email')">
                    &#x24E7;
                  </button>
                </div>
              </div>
        ...
        <tbody>
          <tr
            v-for="(account, index) in visibleIndices"
            :key="account.id"
            class="bg-[#e5e9ec]"
          >
            <td class="px-6 py-4 text-[#2d3e4d] text-center">
              {{ account.name }}
            </td>
            <td class="px-6 py-4 text-[#2d3e4d] text-center">
              {{ account.email }}
            ...
            <td class="px-6 py-4">
              <select
                :disabled="
                  parseInt(permissions[index]) >= permissionLevel &&
                  permissionLevel < 3
                "
                v-model="permissions[index]"
                @change="updatePermission(account, index)"
                class="w-full px-3 py-2 bg-white border border-gray-300 rounded text-[#2d3e4d] cursor-pointer"
              >
                <option value="0">Viewer</option>
                <option value="1">Editor</option>
                <option :disabled="permissionLevel < 3" value="2">Admin</option>
                <option :disabled="permissionLevel < 3" value="3">
                  Main Admin
                </option>
              </select>
            </td>
            <td class="px-6 py-4">
              <div
                v-if="parseInt(permissions[index]) < permissionLevel"
              ...
              >
                ...
                <button
                  v-else
                  class="rounded-md text-sm font-medium outline-none h-9 py-2 bg-yellow-600 hover:bg-yellow-700 text-white px-6"
                  @click="freezeAccount(account)"
                >
                  Unfreeze
                </button>
...
async function deleteAccount(id: string) {
  ...
  if (success) {
    const index = props.accounts.findIndex((account) => account.id === id);
