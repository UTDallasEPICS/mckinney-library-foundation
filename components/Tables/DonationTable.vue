            ...
            <table class="w-full">
                ...
                <tbody>
                    <tr v-for="(row,idx) in sortedIndices" :key="idx" class="hover:bg-[#e8f0f7] transition-colors border-b border-gray-200 cursor-pointer">
                        ...
                        <td class="px-6 py-4 text-[#2d3e4d] text-left text-sm">{{ row.donation.event?.eventName || '' }}</td>
  ...
  return props.data.filter((row) =>
    searchFields.every((field) => {
        ...
        switch(field){
            ...
            case 'event' : value.value = (row?.donation.event && typeof row.donation.event === 'object' ? row.donation.event.eventName : row?.donation.event)?.toString().toLowerCase() ?? ""; break;
            ...
            if (field.active) {
                ...
                switch(field.name) {
                    case 'donorName':
                        ...
                        break;
                    case 'event' : 
                        const aEvent = (a.donation.event && typeof a.donation.event === 'object' ? a.donation.event.eventName : a.donation.event) || '';
                        const bEvent = (b.donation.event && typeof b.donation.event === 'object' ? b.donation.event.eventName : b.donation.event) || '';
